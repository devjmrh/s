...
app = Client("Channel - app",api_id=14170449,api_hash="03488b3c030fe095667e7ca22fe34954",bot_token="7158379495:AAG4MmpxffHB5Rht9Twa6OL1NJEh1LobIas")
...
async def START(c:Client,m:Message):
	...
	ارسل رسالتك وسيتم الرد عليك قريبا
		""",reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("Dev",user_id=820775145)]]),quote= True)
	else:
		db.execute("INSERT INTO USERS(user_id) VALUES(?)", (m.from_user.id,))
		con.commit()
		try:
			await app.send_message(820775145,f"""
		<u>«**New User**»</u>
...
@app.on_message(filters.command("تفعيل التواصل","") & filters.user(820775145) & filters.private)
...
@app.on_message(filters.command("تعطيل التواصل","") & filters.user(820775145) & filters.private)
...
@app.on_message(filters.command("الاحصائيات","") & filters.user(820775145) & filters.private)
...
@app.on_message(filters.command("اذاعه للكل","") & filters.user(820775145) & filters.private)
...
@app.on_message(filters.command("حظر عضو","") & filters.user(820775145) & filters.private)
...
@app.on_message(filters.command("الغاء حظر عضو","") & filters.user(820775145) & filters.private)
...
@app.on_message(filters.private & ~filters.command("start") & ~filters.user(820775145))
async def Private(c:Client,m:Message):
	...
	if CHECK_BAND(user_id=m.from_user.id):
		...
		await app.copy_message(chat_id=820775145,
